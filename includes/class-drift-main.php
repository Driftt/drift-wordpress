...
if ( ! class_exists( 'Drift_Main' ) ) {
  ...
  class Drift_Main {
    ...
    /**
  	 ...
  	 * @since 3.2.9
  	 * @access protected
  	 * @var string $plugin_URL Stores the URL of the plugin.
  	 */
    ...
    */
     ...
     * Sanitizes the input.
     *
     * Check the input settings and sanitizes them.
     *
     * @since 3.2.9
     *
     ...
     *
     * Adds jQuery script in dashboard plugin settings page.
     *
     * @since 3.2.9
     *
    ...
    public function settings_enqueue_script( $hook ) {
      ...
      wp_enqueue_script( 'drift-custom-js', $this->plugin_URL . '/admin/js/custom.min.js' );
    }
    ...
    public function enqueue_script() {
      ...
      if ( is_feed() || is_robots() || is_trackback() ||
           ( is_single() || is_page() ) && in_array( $this_page_id, $hideon_pages ) ||
      ...
      printf( __( '%1$s Start code snippet by Drift plugin %2$s%3$s%1$s End code snippet by Drift plugin %2$s', 'drift' ),
        '<!--', // %1$s
        '-->',  // %2$s
     ...
     *
     * Renders the checkbox input for Drift plugin settings page.
     * Permits or not the identifying of the authenticated user on the site.
     *
     * @since 3.2.9
     *
     * @see get_option function is relied on
    ...
    public function hideon_pages_render() {
      ...
      <label for="hideon_pages">
        <input type="text" name="drift_options[hideon_pages]" placeholder="1,23,456" id="hideon_pages" value="<?php echo isset( $options['hideon_pages'] ) ? $options['hideon_pages'] : ''; ?>" />
        <?php
        /* translators: 1: Opening HTML span tag 2: Closing HTML span tag */
        printf( __( '%1$sYou can add many IDs of posts or pages separated by comma where live chat does not appear.%2$s', 'drift' ),
     ...
     *
     * @link https://app.drift.com/customize
     *
     * @since 3.2.9
     *
     * @see get_option function is relied on
    ...
    public function code_snippet_render() {
      ...
      <p class="description">
        ...
        printf( __( 'Enter your Drift JS code snippet. You can find your Drift JS code snippet %1$shere%2$s. A Free Drift account is required to use this plugin.', 'drift' ),
          '<a href="https://app.drift.com/customize" target="_blank" title="'. __( 'Open Drift Settings', 'drift' ) .'">', // %1$s
          '</a>' // %2$s
        ); ?>
      </p>
      <?php
    ...
    private function identifying_user() {
      ...
      if ( $current_user->user_email ) {
        ...
        /* translators: 1: Opening HTML comment 2: Closing HTML comment 3: HTML JavaScript tag */
        printf( __( '%1$s Start identify call for Drift %2$s%3$s%1$s End identify call for Drift %2$s', 'drift' ),
          '<!--', // %1$s
          '-->', // %2$s
          '<script>
     ...
     *
     * @see version_compare function is relied on
        ...
        if ( isset( $old ) ) {
          ...
          $wpdb->query(
            $wpdb->prepare(
              ...
              WHERE option_name = %s
              ",
              'drift_options', maybe_serialize( $new ), 'Drift_settings'
            )
          );
        }
      }
